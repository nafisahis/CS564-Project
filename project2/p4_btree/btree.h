...
*/
	...
	private:
		...
		// Array to store access path

		PageId accessPath[10];
		...
		const void lookUpInt();
		const void printNode(struct LeafNodeString* lNode);

		const void initializeLeaf(struct LeafNodeInt* lNode);
		const void initializeNonLeaf(struct NonLeafNodeInt* nlNode);
		const void addToParent(const void *key, const PageId pid,const PageId pidr, const int localAccessLevel);
		const void insertIntegerEntry(const void *key, const RecordId rid);
		const void startIntScan(const void* lowValParm,const void* highValParm);
